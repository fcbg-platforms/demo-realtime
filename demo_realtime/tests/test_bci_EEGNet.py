...
def test_invalid_arguments():
    ...
    with pytest.raises(AssertionError):
        EEGNet(
            n_classes=-4, n_channels=64, n_samples=128,
        )
    ...
    with pytest.raises(TypeError, match="'dropoutRate' must be"):
        EEGNet(
            n_classes=4, n_channels=64, n_samples=128, dropoutRate="0.5",
        )
    with pytest.raises(AssertionError):
        EEGNet(
            n_classes=4, n_channels=64, n_samples=128, dropoutRate=1.1,
        )
    with pytest.raises(AssertionError):
        EEGNet(
            n_classes=4, n_channels=64, n_samples=128, dropoutRate=-0.1,
        )

    with pytest.raises(TypeError, match="'kernelLength' must be"):
        EEGNet(
            n_classes=4, n_channels=64, n_samples=128, kernelLength=32.
        )
    with pytest.raises(AssertionError):
        EEGNet(
            n_classes=4, n_channels=64, n_samples=128, kernelLength=-32
