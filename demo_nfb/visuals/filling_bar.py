...
class FillingBar(_Visual):
    ...
    def putBar(
        self, length, width, margin, color, fill_color, fill_perc=0, axis=0
    ):
        """
        ...
        color : str | tuple
            Color used to draw the bar background as a matplotlib string or a
    ...
    def _check_length_margin(length, margin, axis, window_size):
        """
        Checks that the length and margin are strictly positive and add up to a
        shorter dimension than the window dimension along the relevant axis.
        """
        length = int(length)
        margin = int(margin)
        assert 0 < length
